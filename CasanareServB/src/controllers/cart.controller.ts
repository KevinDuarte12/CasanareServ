...
import User from '../db/models/user';  // Cambiado a mayúscula inicial para consistencia
import { Op, Model } from 'sequelize';

// Interfaz extendida de Request para incluir userId
...
export const addToCart = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    ...
    if (cartItem) {
      ...
      const newQuantity = (cartItem.get('quantity') as number) + quantity;
      ...
      cartItem = await ItemCart.create({
        ...
        id_product,
        quantity,
        price: product.get('price') as number  // CORRECCIÓN
      });
