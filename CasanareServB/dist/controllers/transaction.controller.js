 ...
 */
function sendSellerNotificationEmail(sellerEmail, sellerName, buyerName, transactionInfo) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
 ...
 */
function saveInternalNotification(userId, title, message, type, entityType, entityId) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield notifications_1.default.create(notificationData);
...
const createPayment = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        }); // Usar 'as any' temporalmente mientras arreglas los tipos
...
const payuResponse = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const reference = (referenceCode || reference_sale);
        // Obtener estado con cualquier nombre que PayU envíe
        const state = (transactionState || lapTransactionState || polTransactionState || state_pol);
        ...
        if (oldStatus !== dbStatus) {
            ...
            if (response_code_pol || polResponseCode) {
                updateData.payu_response_code = (response_code_pol || polResponseCode);
            }
            if (response_message_pol) {
                updateData.payu_response_message = response_message_pol;
            }
            // Actualizar la transacción
            yield transaction.update(updateData);
            ...
            if (dbStatus === 'completada') {
                ...
                if (cartId) {
                    ...
                    yield cart_1.default.update({ status: 'comprado' }, { where: { id_cart: cartId } });
...
exports.payuResponse = payuResponse;
// Función auxiliar para actualizar stock después del pago (CÓPIALA DE BARTER)
function updateStockAfterPayment(transaction) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const cartId = transaction.get('id_cart');
            if (!cartId) {
            ...
            for (const item of cartItems) {
                try {
                    ...
                    const productId = product.get('id_product');
...
const paymentNotification = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const transaction = yield transaction_1.default.findOne({
            where: { reference_payu: reference },
            include: [
                { model: cart_1.default, as: 'cartInfo' },
                { model: user_1.default, as: 'transactionUser' }
            ]
        });
        if (!transaction) {
            console.error('Transacción no encontrada:', reference);
        ...
        const user = transaction.get('transactionUser');
        ...
        if (oldStatus !== newStatus) {
            ...
            if (user && user.email) {
                yield sendPaymentNotificationEmail(user.email, newStatus, transactionInfo);
            }
            ...
            yield saveInternalNotification(transaction.get('id_user'), notificationTitle, notificationMessage, 'payment');
            ...
            if (newStatus === 'completada') {
                ...
                try {
                    ...
                    for (const [sellerId, data] of sellerProducts) {
                        const seller = data.seller;
                        const products = data.products;
                        const totalForSeller = products.reduce((sum, p) => sum + (p.price * p.quantity), 0);
                        console.log(`📧 Notificando al vendedor ${seller.get('name')} (${seller.get('email')})`);
                        // ✅ ENVIAR EMAIL AL VENDEDOR
                        if (seller.get('email')) {
                            yield sendSellerNotificationEmail(seller.get('email'), seller.get('name'), (user === null || user === void 0 ? void 0 : user.get('name')) || 'Cliente', {
                                reference: reference,
                                amount: totalForSeller,
                                date: transaction.get('transaction_date'),
                                paymentMethod: transaction.get('payment_method'),
                        ...
                        yield saveInternalNotification(sellerId, '🎉 ¡Has realizado una venta!', `Tu producto "${productNames}" ha sido comprado por ${formattedAmount}. Referencia: ${reference}`, 'sale_notification', 'transaction', transaction.get('id_transaction'));
                    }
                }
                catch (sellerNotificationError) {
...
const checkPaymentStatus = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        try {
            const response = yield axios_1.default.post(PAYU_API_URL, payload, {
                headers: {
            ...
            if (response.data.code === 'SUCCESS') {
                ...
                if (payuResult && payuResult.payload) {
                    ...
                    if (oldStatus !== newStatus) {
                        ...
                        yield transaction.update({
                            status: newStatus,
                            payu_transaction_id: ((_b = (_a = payuResult.payload.transactions) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.id) || null,
...
const getPurchasedProducts = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (const transaction of transactions) {
            const cartInfo = transaction.get('cartInfo');
...
const completePayment = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (status === 'APPROVED') {
            const cartId = transaction.get('id_cart');
            if (cartId) {
                yield cart_1.default.update({ status: 'comprado' }, {
                    where: {
                        id_cart: cartId
                    }
                });
 ...
 */
const createWebCheckoutPayment = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id_user, id_cart, delivery_address_id, buyerEmail, buyerName, buyerPhone, total, description = 'Compra en CasanareServ' } = req.body;
        ...
        if (delivery_address_id) {
            deliveryAddressData = yield deliveryAddress_1.default.findByPk(delivery_address_id);
        }
        ...
        const webCheckoutData = {
            url: payuConfig.url,
            reference: reference,
            formData: Object.assign({ merchantId: payuConfig.merchantId, accountId: payuConfig.accountId, description: description, referenceCode: reference, amount: amount, tax: '0', taxReturnBase: '0', currency: currency, signature: signature, test: payuConfig.test, buyerEmail: buyerEmail, buyerFullName: buyerName, responseUrl: payuConfig.responseUrl, confirmationUrl: payuConfig.confirmationUrl }, shippingAddress)
        };
...
exports.createWebCheckoutPayment = createWebCheckoutPayment;
/**
 * Endpoint de confirmación para PayU (webhook)
 * POST /api/transaction/payu-confirmation
 */
const payuConfirmation = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (oldStatus !== newStatus) {
            ...
            let cartItems = (cart === null || cart === void 0 ? void 0 : cart.get('items')) || [];
            if (!cartItems.length && cart) {
                cartItems = yield itemcart_1.default.findAll({
            ...
            const productsList = cartItems.map((item) => {
                ...
                return ({
                    name: ((_a = item.get('product')) === null || _a === void 0 ? void 0 : _a.get('name')) || 'Producto',
                    quantity: item.get('quantity') || 1,
                    price: item.get('unit_price') || item.get('price') || ((_b = item.get('product')) === null || _b === void 0 ? void 0 : _b.get('price')) || 0
                });
            });
            // Preparar datos para la notificación
            const transactionInfo = {
            ...
            if (user && user.get('email')) {
                try {
                    yield sendPaymentNotificationEmail(user.get('email'), newStatus, transactionInfo);
            ...
            switch (newStatus) {
                case 'completada':
                    ...
                    notificationMessage = `Tu pago por ${new Intl.NumberFormat('es-CO', {
                        style: 'currency',
                        currency: 'COP'
                    }).format(transaction.get('total_amount'))} ha sido confirmado. Referencia: ${reference}`;
                    break;
                case 'fallida':
                    notificationTitle = '❌ Pago rechazado';
                    notificationMessage = `Tu pago por ${new Intl.NumberFormat('es-CO', {
                        style: 'currency',
                        currency: 'COP'
                    }).format(transaction.get('total_amount'))} ha sido rechazado. Referencia: ${reference}`;
                    break;
                case 'pendiente':
                    notificationTitle = '⏳ Pago en proceso';
                    notificationMessage = `Tu pago por ${new Intl.NumberFormat('es-CO', {
                        style: 'currency',
                        currency: 'COP'
                    }).format(transaction.get('total_amount'))} está siendo procesado. Referencia: ${reference}`;
                    break;
                case 'reembolsada':
                    ...
                    notificationMessage = `Tu pago por ${new Intl.NumberFormat('es-CO', {
                        style: 'currency',
                        currency: 'COP'
                    }).format(transaction.get('total_amount'))} ha sido reembolsado. Referencia: ${reference}`;
                    break;
            }
            if (notificationTitle) {
                try {
                ...
                try {
                    ...
                    for (const item of cartItems) {
                        ...
                        if (!seller) {
                            ...
                            const productWithSeller = yield product_1.default.findByPk(product.get('id_product'), {
                                include: [{ model: user_1.default, as: 'user', attributes: ['id', 'name', 'email'] }]
                            });
                            seller = productWithSeller === null || productWithSeller === void 0 ? void 0 : productWithSeller.get('user');
                        }
                        if (seller) {
                            const sellerId = seller.get('id');
                            if (!sellerProducts.has(sellerId)) {
                    ...
                    for (const [sellerId, data] of sellerProducts) {
                        ...
                        try {
                            yield saveInternalNotification(sellerId, '🎉 ¡Nueva venta realizada!', `Tu producto "${productNames}" ha sido comprado por ${formattedAmount}. Referencia: ${reference}. Comprador: ${(user === null || user === void 0 ? void 0 : user.get('name')) || 'Cliente'}`, 'sale_notification', 'transaction', transaction.get('id_transaction'));
                            console.log(`✅ Notificación interna creada para vendedor ID: ${sellerId}`);
                        }
                        catch (sellerNotificationError) {
            ...
            if (response.data.code === 'SUCCESS') {
                ...
                if (payuResult && payuResult.payload) {
                    ...
                    if (barterId && typeof barterId === 'number') {
                        barter = yield barter_1.default.findByPk(barterId);
                    }
                    res.json({
                        success: true,
                        transaction: {
...
function updateBarterPaymentStatus(transaction) {
    return __awaiter(this, void 0, void 0, function* () {
        let barterId = null; // ✅ Declarar barterId fuera del try-catch
        try {
            barterId = transaction.get('id_barter');
            const userId = transaction.get('id_user');
            ...
            const [affectedRows] = yield barter_1.default.update(updateData, {
...
const barterPayuResponse = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const { referenceCode, reference_sale, transactionState, lapTransactionState, TX_VALUE, currency, transactionId, transaction_id, state_pol, response_code_pol, response_message_pol } = req.query;
        // Obtener la referencia (PayU puede enviarla con diferentes nombres)
        const reference = (referenceCode || reference_sale);
        const state = (transactionState || lapTransactionState || state_pol);
        const txId = (transactionId || transaction_id);
        if (!reference) {
        ...
        if (response_code_pol) {
            updateData.payu_response_code = response_code_pol;
        }
        if (response_message_pol) {
            updateData.payu_response_message = response_message_pol;
        }
        ...
        if (dbStatus === 'completada') {
            ...
            if (barterId && typeof barterId === 'number') {
                ...
                if (barter) {
                    ...
                    // Actualizar el barter
                    yield barter.update(barterUpdateData);
                    ...
                    if (barter.get('offer_checkout_completed') && barter.get('request_checkout_completed')) {
                        ...
                        for (const userId of userIds) {
                            if (userId) {
                                yield saveInternalNotification(userId, '🎉 Trueque listo para intercambio', 'Ambos usuarios han completado el checkout. El trueque está en proceso.', 'barter_ready');
                            }
                        }
                    }
                    else {
                        ...
                        if (otherUserId) {
                            yield saveInternalNotification(otherUserId, '💰 Pago de trueque completado', 'El otro usuario completó su pago. Ahora completa tu parte del trueque.', 'barter_payment_completed');
                        }
                    }
                }
            }
    ...
    try {
        ...
        if (oldStatus !== newStatus) {
                ...
                if (barterId && typeof barterId === 'number') {
                    ...
                    if (barter) {
                        ...
                        if (Object.keys(paymentUpdateData).length > 0) {
                            ...
                            yield barter.update(paymentUpdateData);
                            ...
                            try {
                                yield (0, barter_controller_1.checkAndUpdateBarterCompletion)(barterId);
                        ...
                        if (offerCompleted && requestCompleted) {
                            ...
                            for (const userId of userIds) {
                                if (userId) {
                                    yield saveInternalNotification(userId, '🎉 Trueque listo para intercambio', 'Ambos usuarios han completado el pago. El trueque está listo.', 'barter_ready', 'barter', barterId);
                            ...
                            const otherUserId = isOfferingUser
                                ...
                                : barter.get('id_user_offer');
                            if (otherUserId) {
                                yield saveInternalNotification(otherUserId, '💰 Pago de trueque completado', 'El otro usuario completó su pago. Ahora completa tu parte del trueque.', 'barter_payment_completed', 'barter', barterId);
            ...
            if (userId) {
                ...
                if (notificationTitle) {
                    // ✅ Crear notificación interna
                    yield saveInternalNotification(userId, notificationTitle, notificationMessage, 'barter_payment', 'transaction', transaction.get('id_transaction'));
 ...
 */
const getSoldProducts = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { userId } = req.params;
        if (!userId) {
            return res.status(400).json({
        ...
        const soldTransactions = yield transaction_1.default.findAll({
            where: {
        ...
        soldTransactions.forEach((transaction) => {
            ...
            const transactionData = {
                ...
                reference_payu: transaction.get('reference_payu'),
                buyer: {
                    id: buyer === null || buyer === void 0 ? void 0 : buyer.get('id'),
                    name: buyer === null || buyer === void 0 ? void 0 : buyer.get('name'),
                    email: buyer === null || buyer === void 0 ? void 0 : buyer.get('email')
                }
            };
            if (cart && cart.items) {
